...
| `npm run verify` | **The pre-press checks, in one command.** Fifteen pass/fail, run against the built pages rather than the sources, plus what it cannot check and says so. `--strict` exits non-zero on any failure, which is what CI uses. |
