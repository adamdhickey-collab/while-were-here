  ...
  <div class="wrap">
    ...
    <div class="spine">
      <div class="spine__stack">
        <p class="spine__text">${esc(book.title)}</p>
...
/* Faces are resolved through Node rather than by joining `root/node_modules`.
   ...
   parent, has no node_modules of its own. Joining the path found nothing there
   and the build quietly fell back to system faces — a proof PDF that is not set
   in the book's type. Node's resolver walks up the way `import` does. */
const require = createRequire(import.meta.url);
function faceFile(spec) {
...
async function installFonts(out, html = '') {
  const dir = path.join(out, 'fonts');
  fs.mkdirSync(dir, { recursive: true });
  let css = '/* GENERATED by scripts/build.mjs. Faces are vendored from npm. */\n';
  for (const f of FONTS) {
    const src = faceFile(f.file);
    const name = path.basename(f.file);
    if (!src) { console.warn(`  ! missing font ${f.file}`); continue; }
    fs.copyFileSync(src, path.join(dir, name));
    css += `@font-face {
  font-family: "${f.family}";
  ...
  const need = unlatinChars(html);
  if (need.size) {
    const cjk = installCjk(dir, need);
    css += cjk.css;
    if (cjk.files) {
      console.log(`  CJK: ${cjk.covered} characters from ${cjk.files} subsets (${(cjk.bytes / 1024).toFixed(0)} KB)`);
    }
  }
  ...
  /* Carries the CJK names too. This overrides the fallback list inside the
     var() in tokens.css, so a stack that omitted them here would lose them. */
  css += `\n:root { --display-family: ${licensed.found
    ? '"Falutin Title", "Fraunces", "Hoefler Text", "Noto Sans JP", "Noto Sans KR", serif'
    : '"Fraunces", "Hoefler Text", "Noto Sans JP", "Noto Sans KR", serif'}; }\n`;
...
export async function build() {
  ...
  const licensedCount = await installFonts(out, Object.values(docs).join('\n'));
  for (const [name, html] of Object.entries(docs)) fs.writeFileSync(path.join(out, name), html);
  writeManifest();
