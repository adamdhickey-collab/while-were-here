...
def main():
    ...
    if suspect:
        ...
        if crops:
            ...
            print("    turned out to be correct was captured 16 Sep – 4 Oct, 2024 or 2025. Every")
            print("    wrong file carried an off-trip date. The date is a better discriminator")
            print("    than the shape and it costs nothing to read.")
            print("")
            print("    HISTORY, so nobody re-learns it: before the rendered crops were exported,")
            print("    matching by name+shape against the originals produced verdicts like ‘a")
            print("    dog on a lawn’ and ‘a hibiscus’ for frames that exist exactly as the")
            print("    document describes them — the name pointed at a different photograph in a")
            print("    library where one number can cover four assets across eleven years. The")
            print("    renders in `photo library edits` carry the right asset identity, so a")
            print("    name+size+date match there is strong. A match against bare originals is")
            print("    still just a shortlist to open.")
